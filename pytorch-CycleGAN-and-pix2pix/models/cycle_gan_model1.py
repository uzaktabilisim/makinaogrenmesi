...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            [self.netD_A, self.netD_B, self.netG_A, self.netG_B], [self.optimizer_D, self.optimizer_G] = amp.initialize(
                [self.netD_A, self.netD_B, self.netG_A, self.netG_B], [self.optimizer_D, self.optimizer_G], opt_level=opt.opt_level, num_losses=3)
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        with amp.scale_loss(loss_D_real, self.optimizer_D, loss_id=0) as loss_D_real_scaled:
            loss_D_real.backward()
        # Fake
        pred_fake = netD(fake.detach())
        loss_D_fake = self.criterionGAN(pred_fake, False)
        with amp.scale_loss(loss_D_fake, self.optimizer_D, loss_id=1) as loss_D_fake_scaled:
            loss_D_fake_scaled.backward()
        # Combined loss and calculate gradients
        loss_D = (loss_D_fake_scaled + loss_D_real_scaled) * 0.5
        loss_D.backward()
        return loss_D
    ...
    def backward_G(self):
        ...
        with amp.scale_loss(loss_G, self.optimizer_G, loss_id=2) as loss_G_scaled:
