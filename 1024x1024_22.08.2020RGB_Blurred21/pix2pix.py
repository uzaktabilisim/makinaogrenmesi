...
def generate_images(model, test_input, tar, showResults=1):
...
webcam = VideoCapture('./../videodataset/2deneme.mp4')
...
result = cv2.VideoWriter('fulllayer2.avi',
...
while success:
    ...
    input_image = load_from_video(input_image)
    input_image = np.asarray([input_image])
    print(input_image.shape)

    frame = generate_images(generator, input_image, input_image, 0)
    pil_image = tf.keras.preprocessing.image.array_to_img(frame)
    imtemp = pil_image.copy()
    print(imtemp)
    result.write(cv2.cvtColor(np.array(imtemp), cv2.COLOR_RGB2BGR))
