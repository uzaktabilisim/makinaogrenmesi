...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Enhancing the Spatial Resolution of Stereo Images using a Parallax Prior')
    ...
    parser.add_argument('--scale_factor', type=int,
                        default=2, help='scale factor')
    parser.add_argument('--train_data_size', type=int,
                        default=20000, help='train_data_size')
    ...
    for epoch in range(epoch_size):
        ...
        for i, data in enumerate(dataloder, 0):
            input, target = data[0].to(device, dtype=torch.float), data[1].to(
